...
export default function AgentsPage() {
  ...
  const [agents, setAgents] = useState([
    ...
    },
    {
      ...
      memoryCount: 5,
      storageProvider: 'storacha',
      description: 'Technical assistant with limited access',
    },
    {
      id: 'agent-003',
      name: 'You',
    ...
    <div className='flex-1 space-y-4 p-4 md:p-8 pt-6'>
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground'>
              {agents.filter((a) => a.type === 'ai').length} AI,{' '}
              {agents.filter((a) => a.type === 'human').length} human
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {new Set(agents.map((a) => a.storageProvider)).size}
            </div>
            <p className='text-xs text-muted-foreground'>
              Active providers in use
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {agents.reduce((acc, agent) => acc + agent.memoryCount, 0)}
            </div>
            <p className='text-xs text-muted-foreground'>
      ...
      <Tabs defaultValue='all' className='space-y-4'>
        ...
        <TabsContent value='all' className='space-y-4'>
          <Card>
            <CardHeader className='pb-3'>
              ...
              <CardDescription>
                Manage your AI agents and their access to memories
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value='ai' className='space-y-4'>
          <Card>
            ...
            <CardContent className='p-0'>
              <AgentsTable
                agents={agents.filter((agent) => agent.type === 'ai')}
                setAgents={setAgents}
              />
            </CardContent>
          </Card>
        ...
        <TabsContent value='human' className='space-y-4'>
          <Card>
            ...
            <CardContent className='p-0'>
              <AgentsTable
                agents={agents.filter((agent) => agent.type === 'human')}
                setAgents={setAgents}
              />
            </CardContent>
          </Card>
