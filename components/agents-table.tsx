...
import { Bot, User, MoreHorizontal, Shield, Calendar, Key, Database, Brain, Sparkles } from "lucide-react"
...
export function AgentsTable({ agents, setAgents }: AgentsTableProps) {
  ...
  return (
    <div>
            ...
            <TableRow key={agent.id} className="group">
              ...
              <TableCell>
                ...
                >
                  ...
                  {agent.accessLevel === "full" ? "Full" : agent.accessLevel === "limited" ? "Limited" : "Read-Only"}
                </Badge>
              </TableCell>
              <TableCell>
                <div className="flex items-center">
                  {getStorageIcon(agent.storageProvider)}
                  <span className="ml-2">{getStorageName(agent.storageProvider)}</span>
                </div>
              </TableCell>
              <TableCell>
                <Switch
      ...
      {selectedAgent && <AgentDetailsDialog agent={selectedAgent} open={detailsOpen} onOpenChange={setDetailsOpen} />}
    </div>
  )
}
